...
Advanced Parameters 
*******************
Many workflow-specific and general knobs can be tuned for self-supervision. Below is a practical summary using the current options in `config.py <https://github.com/BiaPyX/BiaPy/blob/master/biapy/config/config.py>`__.

**General tuning parameters (very useful in practice)**

* **Model architecture** (``MODEL.ARCHITECTURE``): Backbone network. Current options for self-supervision are ``unet``, ``resunet``, ``resunet++``, ``attention_unet``, ``multiresunet``, ``seunet``, ``resunet_se``, ``unetr``, ``edsr``, ``rcan``, ``dfcan``, ``wdsr``, ``vit``, ``mae``, ``unext_v1``, ``unext_v2``, ``hrnet`` and ``stunet``. Default: ``unet``.
...
**Self-supervision specific options**

* ``PROBLEM.SELF_SUPERVISED.PRETEXT_TASK``: Pretext task. Options: ``crappify`` and ``masking``. Default: ``crappify``.

* ``PROBLEM.SELF_SUPERVISED.RESIZING_FACTOR``: Down/up-sampling factor used by ``crappify``. Default: ``4``.

* ``PROBLEM.SELF_SUPERVISED.NOISE``: Noise level injected in ``crappify``. Default: ``0.2``.

* ``LOSS.TYPE``: For ``crappify``, supported options are ``MAE``, ``MSE``, ``SSIM``, ``W_MAE_SSIM`` and ``W_MSE_SSIM`` (automatic default is ``MAE`` when empty). For ``masking``, loss is set automatically by the workflow (MSE). Default in config: ``""`` (automatic selection).

* ``MODEL.VIT_MODEL``: ViT preset used by transformer-based self-supervised backbones. Options: ``custom``, ``vit_base_patch16``, ``vit_large_patch16``, ``vit_huge_patch16``. Default: ``custom``.

* ``MODEL.MAE_MASK_TYPE`` and ``MODEL.MAE_MASK_RATIO``: MAE masking strategy and masking ratio (applied when using MAE-style training). Defaults: ``grid`` and ``0.5``.
